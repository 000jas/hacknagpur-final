...
# Alert counter file for real-time dashboard updates
PROJECT_DIR = Path(__file__).parent.parent
ALERT_COUNTER_FILE = PROJECT_DIR / "alert_counter.json"
BEEP_SOUND_FILE = PROJECT_DIR / "beep.mp3"

# Real-time alert counter
REALTIME_ALERT_COUNT = 0
...
def increment_alert_counter():
    """Increment the real-time alert counter for dashboard."""
    global REALTIME_ALERT_COUNT
    REALTIME_ALERT_COUNT += 1
    try:
        with open(ALERT_COUNTER_FILE, 'w') as f:
            json.dump({
                "count": REALTIME_ALERT_COUNT,
                "last_updated": time.time()
            }, f)
        print(f"[COUNTER] Alert count incremented to {REALTIME_ALERT_COUNT}")
    except Exception as e:
        print(f"[ERROR] Failed to update alert counter: {e}")

def reset_alert_counter():
    """Reset the alert counter to zero."""
    global REALTIME_ALERT_COUNT
    REALTIME_ALERT_COUNT = 0
    try:
        with open(ALERT_COUNTER_FILE, 'w') as f:
        ...
        print(f"[COUNTER] Alert count reset to 0")
    except Exception as e:
        print(f"[ERROR] Failed to reset alert counter: {e}")

def play_alert_sound():
    """Play beep sound for harassment alert."""
    if not AUDIO_ENABLED:
        return
    
    try:
        if BEEP_SOUND_FILE.exists():
            pygame.mixer.music.load(str(BEEP_SOUND_FILE))
            pygame.mixer.music.play()
            print(f"[AUDIO] 🔊 Playing alert sound: {BEEP_SOUND_FILE.name}")
        else:
            print(f"[WARNING] Beep file not found: {BEEP_SOUND_FILE}")
    except Exception as e:
...
def alert_harassment(person_id, risk_score, frame, bbox=None):
    ...
    # Increment real-time alert counter for dashboard
...
def run_harassment_detection(video_source=0):
    ...
    # Reset alert counter when video ends
    reset_alert_counter()
