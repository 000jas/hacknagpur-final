...
def render_system_status():
    ...
    # Read real-time alert counter (for live detection)
    alert_counter_file = os.path.join(PROJECT_DIR, "alert_counter.json")
    realtime_count = 0
    try:
        if os.path.exists(alert_counter_file):
            with open(alert_counter_file, 'r') as f:
                counter_data = json.load(f)
                realtime_count = counter_data.get("count", 0)
    except:
        realtime_count = 0
    
    # Use real-time count if detection is running, otherwise use historical count
    alerts_count = realtime_count if is_running else len(alerts_list)
...
def render_features():
    ...
    with col1:
        ...
        <div class="feature-card">
            ...
            <p>Harassment and crime detection using YOLOv8 pose estimation and XGBoost classification with 85% accuracy.</p>
        </div>
        """, unsafe_allow_html=True)
